        ...
        if result.returncode != 0:
            ...
            if "to modify pip, please run the following command" in stderr_lower:
                ...
                try:
                    ...
                    upgrade_result = subprocess.run(upgrade_cmd, capture_output=True, text=True, timeout=600)
                    
                    if upgrade_result.returncode == 0:
                        print("Pip upgraded successfully, retrying original command...")
                        # Retry the original command
                        retry_result = subprocess.run(cmd, capture_output=True, text=True, timeout=3600)
