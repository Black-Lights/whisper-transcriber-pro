...
class SettingsManager:
    ...
    def import_settings(self, file_path):
        ...
        try:
            ...
            # Validate and merge
            self.settings = self.deep_merge(
                self.default_settings.copy(), imported_settings
            )
            return self.save_settings()
        except Exception as e:
    ...
    def validate_settings(self):
        ...
        for category, key, min_val, max_val in numeric_settings:
            ...
            if not isinstance(value, (int, float)) or not (min_val <= value <= max_val):
                errors.append(
                    f"Invalid {category}.{key}: {value} (should be {min_val}-{max_val})"
                )
    ...
    def get_transcription_options(self):
        ...
        return {
            ...
            "merge_short": self.get_setting("output", "text_processing")[
                "merge_short_segments"
            ],
            "word_timestamps": self.get_setting("output", "text_processing")[
                "word_timestamps"
            ],
    ...
    def update_from_ui(self, ui_values):
        ...
        # General settings
        self.set_setting(
            "general", "default_model", ui_values.get("model_size", "medium")
        )
        self.set_setting(
            "general", "default_language", ui_values.get("language", "auto")
        )
