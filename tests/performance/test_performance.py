...
from src.utils import format_file_size, get_file_info
...
class TestPerformance:
    ...
    def test_file_info_extraction_performance(self, temp_dir):
        ...
        assert (
            processing_time < 1.0
        ), f"File info extraction took {processing_time:.2f}s"
    ...
    def test_model_cache_size_calculation_performance(self, temp_dir):
        ...
        try:
            ...
            ), f"Cache calculation took {calculation_time:.2f}s"
    ...
    def test_environment_check_performance(self, temp_dir):
        ...
        try:
            ...
            scripts_dir = env_manager.venv_dir / (
                "Scripts" if env_manager.is_windows else "bin"
            )
            scripts_dir.mkdir(parents=True, exist_ok=True)

            python_exe = scripts_dir / (
                "python.exe" if env_manager.is_windows else "python"
            )
...
class TestMemoryPerformance:
    ...
    def test_memory_cleanup_after_transcription(self, mock_env_manager, temp_dir):
        ...
        with patch("subprocess.Popen") as mock_popen, patch(
            ...
            "json.loads", return_value=mock_result
        ):
...
class TestScalabilityPerformance:
    ...
    def test_scalability_with_many_models(self, temp_dir):
        ...
        try:
            ...
            assert (
                processing_time < 1.0
            ), f"Processing {len(all_models)} models took {processing_time:.2f}s"
    ...
    def test_scalability_with_many_settings(self, temp_dir):
        ...
        assert (
            processing_time < 5.0
        ), f"Processing {total_settings} settings took {processing_time:.2f}s"
    ...
    def test_concurrent_transcription_operations(self, mock_env_manager, temp_dir):
        ...
        assert (
            avg_worker_time < 2.0
        ), f"Average worker time {avg_worker_time:.2f}s too slow"
...
class TestRealWorldPerformance:
    ...
    @pytest.mark.performance
    @pytest.mark.slow
    def test_realistic_transcription_workflow_performance(
        self, mock_env_manager, temp_dir
    ):
        ...
        with patch("subprocess.Popen") as mock_popen, patch(
            ...
            mock_process = Mock()
            mock_process.poll.side_effect = [None] * 10 + [
                0
            ...
            result = engine.transcribe(
                str(audio_file), str(temp_dir), options, progress_callback
            )
        ...
        ), f"Realistic workflow took {total_workflow_time:.2f}s (too slow)"

        # Check update frequency
        if len(progress_updates) > 1:
            update_times = [update[0] for update in progress_updates]
            avg_update_interval = (update_times[-1] - update_times[0]) / (
                len(update_times) - 1
            )
            assert (
                avg_update_interval < 5.0
            ), f"Average update interval {avg_update_interval:.2f}s too slow"
