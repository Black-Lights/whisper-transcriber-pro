...
class TestCompleteWorkflow:
    ...
    def setup_managers(self, temp_dir):
        ...
        model_manager.models = {
            "tiny": {
                "size": "39 MB",
                "description": "Fastest",
                "url": "http://example.com/tiny.pt",
                "sha256": "abc123",
            },
            "medium": {
                "size": "769 MB",
                "description": "Recommended",
                "url": "http://example.com/medium.pt",
                "sha256": "def456",
            },
        }
    ...
    def test_complete_transcription_workflow_success(
        self, setup_managers, mock_audio_file, mock_whisper_result
    ):
        ...
        with patch.object(
            transcription_engine, "run_enhanced_transcription"
        ) as mock_transcribe:
            mock_transcribe.return_value = mock_whisper_result

            # Mock file generation
            with patch.object(
                transcription_engine, "generate_output_files"
            ) as mock_generate:
                output_files = [
                ...
                result = transcription_engine.transcribe(
                    str(mock_audio_file), str(output_dir), options
                )
    ...
    def test_workflow_error_handling_missing_file(self, setup_managers):
        ...
        result = transcription_engine.transcribe(
            "/nonexistent/file.mp3", str(temp_dir / "output"), options
        )

        assert result["success"] is False
        assert "not found" in result["error"].lower()

    def test_workflow_error_handling_transcription_failure(
        self, setup_managers, mock_audio_file
    ):
        ...
        with patch.object(
            ...
            mock_transcribe.side_effect = Exception(
                "Transcription failed due to model error"
            )
            ...
            result = transcription_engine.transcribe(
                str(mock_audio_file), str(temp_dir / "output"), options
            )
    ...
    def test_workflow_dots_only_detection(self, setup_managers, mock_audio_file):
        ...
        with patch.object(
            ...
            assert result["success"] is False
            assert (
                "silence" in result["error"].lower()
                or "unclear" in result["error"].lower()
            )
    ...
    def test_workflow_stop_functionality(self, setup_managers, mock_audio_file):
        ...
        with patch.object(
            ...
            side_effect=mock_long_transcription,
        ):
            options = {"model_size": "medium", "output_formats": {"text": True}}
            ...
            def transcribe_worker():
                result_container["result"] = transcription_engine.transcribe(
                    str(mock_audio_file), str(temp_dir), options
                )
    ...
    def test_workflow_multiple_output_formats(
        self, setup_managers, mock_audio_file, mock_whisper_result
    ):
    ...
    def test_workflow_with_live_updates(
        self, setup_managers, mock_audio_file, mock_whisper_result
    ):
        ...
        with patch.object(
            transcription_engine,
            "run_enhanced_transcription",
            side_effect=mock_transcription_with_live_updates,
        ):
            with patch.object(
                transcription_engine, "generate_output_files"
            ) as mock_generate:
    ...
    def test_workflow_error_recovery(self, setup_managers, mock_audio_file):
        ...
        def failing_then_succeeding_transcription(*args, **kwargs):
            ...
            return {
                ...
                "segments": [
                    {"start": 0, "end": 1, "text": "Success", "avg_logprob": -0.1}
                ],
                "duration": 1.0,
            }
        ...
        with patch.object(
            transcription_engine,
            "run_enhanced_transcription",
            side_effect=failing_then_succeeding_transcription,
        ):
            options = {"model_size": "medium", "output_formats": {"text": True}}
    ...
    def test_workflow_performance_monitoring(
        ...
        with patch.object(
            transcription_engine,
            "run_enhanced_transcription",
            side_effect=timed_transcription,
        ):
            with patch.object(
                transcription_engine, "generate_output_files"
            ) as mock_generate:
...
class TestWorkflowIntegrationEdgeCases:
    ...
    def test_workflow_with_empty_audio(self, temp_dir):
        ...
        result = transcription_engine.transcribe(
            str(empty_file), str(temp_dir), options
        )
    ...
    def test_workflow_resource_cleanup(self, setup_managers, mock_audio_file):
        ...
        with patch.object(
            transcription_engine,
            "run_enhanced_transcription",
            side_effect=mock_transcription_with_temp_files,
        ):
            with patch.object(
                transcription_engine, "generate_output_files"
            ) as mock_generate:
...
class TestWorkflowRealWorldScenarios:
        ...
        with patch.object(model_manager, "cache_dir", temp_dir):
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        model_manager, "check_downloaded_models"
                    ) as mock_check:
                        mock_check.return_value = {
                            "medium": {
                                "valid": True,
    ...
    def test_user_workflow_batch_processing(self, setup_managers, temp_dir):
        ...
        with patch.object(
            transcription_engine,
            "run_enhanced_transcription",
            side_effect=mock_batch_transcription,
        ):
            with patch.object(
                transcription_engine, "generate_output_files"
            ) as mock_generate:

                def mock_file_generation(
                    result, base_filename, output_dir, options, progress_callback=None
                ):
                ...
                for audio_file in audio_files:
                    result = transcription_engine.transcribe(
                        str(audio_file), str(temp_dir / "output"), options
                    )
                    results.append(result)
    ...
    def test_user_workflow_settings_persistence(self, temp_dir):
        ...
        with patch.object(SettingsManager, "settings_file", settings_file):
            with patch("pathlib.Path.exists", return_value=True):
                with patch(
                    "builtins.open", mock_open(read_data=json.dumps(saved_settings))
                ):
    ...
    def test_user_workflow_error_recovery_and_retry(self, setup_managers, temp_dir):
        ...
        for i, error_msg in enumerate(failure_scenarios):
            with patch.object(
                ...
                result = transcription_engine.transcribe(
                    str(audio_file), str(temp_dir), options
                )
    ...
    def test_user_workflow_progress_monitoring(self, setup_managers, temp_dir):
        ...
        def mock_detailed_transcription(*args, **kwargs):
            ...
            return {
                ...
                "segments": [
                    {"start": 0, "end": 10, "text": "Long content", "avg_logprob": -0.2}
                ],
                "duration": 10.0,
            }
        ...
        with patch.object(
            transcription_engine,
            "run_enhanced_transcription",
            side_effect=mock_detailed_transcription,
        ):
            with patch.object(
                transcription_engine, "generate_output_files"
            ) as mock_generate:
