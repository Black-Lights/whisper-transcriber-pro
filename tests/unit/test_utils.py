...
class TestUtilityFunctions:
    ...
    def test_get_file_info_success(self, mock_duration, mock_stat, mock_exists):
        ...
        mock_stat.return_value = Mock(
            st_size=1024 * 1024 * 5, st_mtime=1640995200  # 5MB  # 2022-01-01 00:00:00
        )
    ...
    def test_get_file_info_no_duration(self, mock_stat, mock_exists):
        ...
        mock_exists.return_value = True
        mock_stat.return_value = Mock(st_size=1024 * 500, st_mtime=1640995200)  # 500KB
    ...
    def test_get_system_info_with_psutil(
        self, mock_disk, mock_memory, mock_cpu_percent, mock_cpu_count
    ):
        """Test system info collection with psutil"""
        mock_cpu_count.return_value = 8
        mock_cpu_percent.return_value = 25.5
        mock_memory.return_value = Mock(
            total=8 * 1024**3, available=4 * 1024**3  # 8GB  # 4GB available
        )
        mock_disk.return_value = Mock(
            total=500 * 1024**3, free=200 * 1024**3  # 500GB  # 200GB free
        )
    ...
    def test_create_output_filename_with_suffix(self, temp_dir):
        ...
        result = create_output_filename(
            "/path/to/audio.mp3", str(temp_dir), "detailed", ".json"
        )
    ...
    def test_cleanup_temp_files_exception(
        self, mock_get_temp, mock_exists, mock_rmtree
    ):
