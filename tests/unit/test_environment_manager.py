...
class TestEnvironmentManager:
    ...
    def test_setup_environment_fresh_install(self, mock_run, mock_venv_create):
        ...
        with patch.object(env_manager.venv_dir, 'exists', return_value=False), \
             ...
             patch.object(env_manager, 'install_cpu_packages') as mock_install:
    ...
    def test_setup_environment_with_gpu(self, mock_run, mock_venv_create):
        ...
        with patch.object(env_manager.venv_dir, 'exists', return_value=False), \
             patch.object(env_manager, 'detect_gpu_support', return_value=True), \
             patch.object(env_manager, 'install_gpu_packages') as mock_install:
    ...
    def test_detect_gpu_support_nvidia(self, mock_run):
        ...
        assert result is True
        mock_run.assert_called_once_with(["nvidia-smi"], capture_output=True, timeout=10)
    ...
    def test_run_pip_command_upgrade_notice(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=1, 
            stderr="WARNING: A new release of pip is available",
            stdout=""
        )
        
        # This should not raise an exception for upgrade notices
        with patch.object(env_manager, 'run_pip_command', wraps=env_manager.run_pip_command):
            try:
    ...
    def test_get_environment_info(self, mock_json_loads, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout='[{"name": "package", "version": "1.0"}]')
    ...
    def test_setup_environment_progress_callback(self, mock_run):
        ...
        with patch.object(env_manager.venv_dir, 'exists', return_value=False), \
             patch.object(env_manager, 'detect_gpu_support', return_value=False), \
             patch.object(env_manager, 'install_cpu_packages'), \
             patch('venv.create'):
    ...
    def test_setup_environment_with_existing_broken_venv(self, mock_run):
        ...
        with patch.object(env_manager.venv_dir, 'exists', return_value=True), \
             patch.object(env_manager, 'check_environment') as mock_check, \
             patch.object(env_manager, 'remove_environment') as mock_remove, \
             patch.object(env_manager, 'detect_gpu_support', return_value=False), \
             patch.object(env_manager, 'install_cpu_packages'), \
             patch('venv.create'):
    ...
    def test_pip_upgrade_restriction_handling(self, mock_run):
        ...
        mock_run.side_effect = [
            Mock(returncode=1, stderr="to modify pip, please run the following command"),
