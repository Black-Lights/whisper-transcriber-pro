...
class TestModelManager:
    ...
    def test_check_downloaded_models_with_valid_models(self, mock_stat, mock_exists):
        ...
        mock_stat.return_value = Mock(
            st_size=1024 * 1024 * 100, st_mtime=time.time()
        )  # 100MB
    ...
    def test_download_models_default(self):
        ...
        with patch.object(
            manager, "download_single_model", return_value=True
        ) as mock_download:
    ...
    def test_download_single_model_success(self, mock_mkdir, mock_get):
        ...
        with patch.object(
            manager, "verify_model_file_by_path", return_value=True
    ...
    def test_download_single_model_already_exists(self, mock_get):
        ...
        with patch.object(model_file, "exists", return_value=True), patch.object(
            manager, "verify_model_file", return_value=True
        ):
    ...
    def test_download_single_model_corrupted_existing(self, mock_get):
        ...
        with patch.object(model_file, "exists", return_value=True), patch.object(
            manager, "verify_model_file", return_value=False
        ), patch.object(model_file, "unlink"), patch.object(
            manager, "verify_model_file_by_path", return_value=True
        ), patch(
    ...
    def test_download_single_model_verification_failure(self, mock_mkdir, mock_get):
        ...
        with patch.object(
            manager, "verify_model_file_by_path", return_value=False
        ), patch("builtins.open", mock_open()), patch(
            "pathlib.Path.exists", return_value=False
        ), patch(
            "pathlib.Path.unlink"
    ...
    @patch("os.unlink")
    def test_download_model_whisper_native_success(
        self, mock_unlink, mock_run, mock_temp
    ):
        ...
        mock_run.return_value = Mock(
            returncode=0, stderr="", stdout="Model loaded successfully!"
        )
    ...
    @patch("subprocess.run")
    @patch("os.unlink")
    def test_download_model_whisper_native_failure(
        self, mock_unlink, mock_run, mock_temp
    ):
