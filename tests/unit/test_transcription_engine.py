...
class TestTranscriptionEngine:
    ...
    def test_is_dots_only_output_edge_cases(self, mock_env_manager):
        ...
        with patch.object(
            engine, "is_dots_only_output", side_effect=Exception("Test error")
        ):
            try:
    ...
    def test_transcribe_success(
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            "builtins.open", mock_open(read_data=result_file_content)
        ), patch("pathlib.Path.unlink"), patch.object(
            engine, "generate_output_files", return_value=["test.txt"]
        ):
    ...
    @patch("subprocess.Popen")
    def test_transcribe_user_stopped(
        self, mock_popen, mock_env_manager, sample_audio_file, temp_dir
    ):
    ...
    @patch("subprocess.Popen")
    def test_transcribe_dots_only_result(
        self, mock_popen, mock_env_manager, sample_audio_file, temp_dir
    ):
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            "builtins.open", mock_open(read_data=result_content)
    ...
    def test_create_enhanced_transcription_script(
        self, mock_env_manager, sample_audio_file
    ):
        ...
        script = engine.create_enhanced_transcription_script(
            str(sample_audio_file), options
        )
    ...
    def test_create_enhanced_transcription_script_auto_language(
        self, mock_env_manager, sample_audio_file
    ):
        ...
        assert (
            "language" not in script or 'transcribe_options["language"]' not in script
        )
    ...
    def test_run_with_live_monitoring_success(self, mock_env_manager):
        ...
        with patch("subprocess.Popen", return_value=mock_process):
            result = engine.run_with_live_monitoring(
                ["python", "script.py"], mock_progress_callback, time.time(), {}
            )
    ...
    def test_run_with_live_monitoring_with_stop(self, mock_env_manager):
        ...
        with patch("subprocess.Popen", return_value=mock_process):
            result = engine.run_with_live_monitoring(
                ["python", "script.py"], None, time.time(), {}
            )

        mock_process.terminate.assert_called()

    def test_generate_output_files_all_formats(
        self, mock_env_manager, temp_dir, mock_whisper_result
    ):
        ...
        with patch("builtins.open", mock_open()) as mock_file:
            files = engine.generate_output_files(
                mock_whisper_result, "test_audio", temp_dir, options
            )
    ...
    def test_generate_output_files_selective_formats(
        self, mock_env_manager, temp_dir, mock_whisper_result
    ):
    ...
    def test_generate_detailed_transcript(
        self, mock_env_manager, temp_dir, mock_whisper_result
    ):
        ...
        with patch("builtins.open", mock_open()) as mock_file:
            engine.generate_detailed_transcript(
                mock_whisper_result, output_file, options
            )
    ...
    def test_error_handling_in_transcription(
        self, mock_env_manager, sample_audio_file, temp_dir
    ):
...
class TestTranscriptionEngineIntegration:
    ...
    def test_full_transcription_workflow_mock(
        self, mock_env_manager, sample_audio_file, temp_dir
    ):
    ...
    def test_live_monitoring_threading(self, mock_env_manager):
        ...
        with patch("subprocess.Popen", return_value=mock_process):
            result = engine.run_with_live_monitoring(
                ["python", "test.py"], mock_progress_callback, time.time(), {}
            )

        assert result["returncode"] == 0
        assert len(events) >= 2  # Should have received multiple updates

    def test_script_generation_with_various_options(
        self, mock_env_manager, sample_audio_file
    ):
