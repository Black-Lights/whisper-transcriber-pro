...
from src.utils import format_time, get_file_info
...
class WhisperTranscriberGUI:
    ...
    def setup_styles(self):
        ...
        style.configure(
            "Time.TLabel", font=("Segoe UI", 11, "bold"), foreground="#3498db"
        )
        style.configure(
            "LiveData.TLabel", font=("Segoe UI", 10, "bold"), foreground="#e67e22"
        )
        ...
        style.configure(
            "Live.TFrame", relief="solid", borderwidth=1, background="#ecf0f1"
        )
    ...
    def create_header(self, parent):
        ...
        title_label = ttk.Label(
            header_frame, text="Whisper Transcriber Pro", style="Title.TLabel"
        )
        title_label.pack()

        subtitle_label = ttk.Label(
            header_frame,
            text="Professional Audio & Video Transcription with Live Updates",
            style="Info.TLabel",
        )
    ...
    def create_live_transcription_section(self, parent):
        ...
        live_frame = ttk.LabelFrame(
            parent,
            text="Live Transcription Monitor",
            padding="10",
        ...
        self.transcription_status = ttk.Label(
            status_frame, text="Ready to start transcription", style="Heading.TLabel"
        )
        self.transcription_status.pack(side=tk.LEFT)

        # Live indicator (hidden until active)
        self.live_indicator = ttk.Label(
            status_frame, text="🔴 LIVE", style="LiveData.TLabel"
        )
        ...
        ttk.Label(
            pos_header, text="Current Position:", font=("Segoe UI", 10, "bold")
        ).pack(side=tk.LEFT)
        ...
        ttk.Label(
            segment_frame, text="Segment Progress:", font=("Segoe UI", 10, "bold")
        ).pack(side=tk.LEFT)

        self.segment_label = ttk.Label(
            segment_frame, text="0 of 0", font=("Segoe UI", 10), foreground="#27ae60"
        )
        ...
        ttk.Label(
            confidence_frame, text="Confidence:", font=("Segoe UI", 10, "bold")
        ).pack(side=tk.LEFT)

        self.confidence_bar = ttk.Progressbar(
            confidence_frame, mode="determinate", length=120, maximum=100
        )
        self.confidence_bar.pack(side=tk.LEFT, padx=(10, 5))

        self.confidence_label = ttk.Label(
            confidence_frame, text="--", font=("Segoe UI", 9), foreground="#34495e"
        )
        ...
        ttk.Label(eta_frame, text="ETA:", font=("Segoe UI", 10, "bold")).pack(
            side=tk.LEFT
        )
        ...
        text_frame = ttk.LabelFrame(
            live_frame, text="Live Transcribed Text", padding="5"
        )
        ...
        self.live_text.tag_configure(
            "high_confidence", foreground="#27ae60", font=("Segoe UI", 11)
        )
        self.live_text.tag_configure(
            "medium_confidence", foreground="#f39c12", font=("Segoe UI", 11)
        )
        self.live_text.tag_configure(
            "low_confidence", foreground="#e74c3c", font=("Segoe UI", 11)
        )
        self.live_text.tag_configure("current_segment", background="#ecf0f1")
        self.live_text.tag_configure(
            "timestamp", foreground="#3498db", font=("Segoe UI", 9, "italic")
        )
        self.live_text.tag_configure(
            "completion", foreground="#27ae60", font=("Segoe UI", 12, "bold")
        )
        self.live_text.tag_configure(
            "processing", foreground="#95a5a6", font=("Segoe UI", 10, "italic")
        )
        ...
        ttk.Button(
            text_buttons,
            text="Copy Text",
            command=self.copy_live_text,
            style="Secondary.TButton",
        ).pack(side=tk.LEFT, padx=(5, 0))
        ttk.Button(
            text_buttons,
            text="Save Text",
            command=self.save_live_text,
            style="Secondary.TButton",
        ).pack(side=tk.LEFT, padx=(5, 0))
    ...
    def create_file_section(self, parent):
        ...
        self.file_entry = ttk.Entry(
            input_frame, textvariable=self.input_file, font=("Segoe UI", 9)
        )
        ...
        self.file_info_label = ttk.Label(
            file_frame, text="No file selected", style="Info.TLabel"
        )
    ...
    def create_model_section(self, parent):
        ...
        ttk.Label(model_row, text="Model:", width=10, font=("Segoe UI", 9)).pack(
            side=tk.LEFT
        )
        ...
        ttk.Label(lang_row, text="Language:", width=10, font=("Segoe UI", 9)).pack(
            side=tk.LEFT
        )
        ttk.Combobox(
            lang_row,
        ...
        ttk.Label(device_row, text="Device:", width=10, font=("Segoe UI", 9)).pack(
            side=tk.LEFT
        )
        ttk.Radiobutton(
            device_row, text="GPU", variable=self.device_type, value="gpu"
        ).pack(side=tk.LEFT)
        ttk.Radiobutton(
            device_row, text="CPU", variable=self.device_type, value="cpu"
    ...
    def create_output_section(self, parent):
        ...
        self.output_entry = ttk.Entry(
            dir_frame, textvariable=self.output_dir, font=("Segoe UI", 9)
        )
        ...
        ttk.Checkbutton(
            formats_frame, text="Plain Text", variable=self.output_formats["text"]
        ).grid(row=0, column=0, sticky=tk.W, padx=(0, 10))
        ttk.Checkbutton(
            formats_frame, text="Detailed", variable=self.output_formats["detailed"]
        ).grid(row=0, column=1, sticky=tk.W)
        ttk.Checkbutton(
            formats_frame, text="SRT Subtitles", variable=self.output_formats["srt"]
        ).grid(row=1, column=0, sticky=tk.W, padx=(0, 10), pady=(5, 0))
        ttk.Checkbutton(
            formats_frame, text="VTT Subtitles", variable=self.output_formats["vtt"]
    ...
    def create_advanced_section(self, parent):
        ...
        ttk.Checkbutton(
            advanced_frame,
            text="Clean text (remove filler words)",
            variable=self.clean_text,
        ).pack(anchor=tk.W, pady=1)

        self.merge_short = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            advanced_frame, text="Merge short segments", variable=self.merge_short
        ).pack(anchor=tk.W, pady=1)

        self.word_timestamps = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            advanced_frame, text="Word-level timestamps", variable=self.word_timestamps
        ).pack(anchor=tk.W, pady=1)
    ...
    def create_progress_section(self, parent):
        ...
        self.progress_label = ttk.Label(
            info_frame, text="Ready to start", style="Info.TLabel"
        )
    ...
    def create_control_buttons(self, parent):
        ...
        ttk.Button(
            ...
            style="Secondary.TButton",
        ).pack(side=tk.RIGHT, fill=tk.X, expand=True, padx=(3, 0))
    ...
    def create_footer(self, parent):
        ...
        status_label = ttk.Label(
            footer_frame, textvariable=self.status_var, style="Info.TLabel"
        )
            ...
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    if (
                        ...
                        and any(
                            "temp_transcribe" in arg for arg in proc.info["cmdline"]
                        )
                    ):
                        proc.terminate()
                        print(
                            f"Terminated zombie transcription process: {proc.info['pid']}"
                        )
    ...
    def save_live_text(self):
        ...
        try:
            ...
            if filename:
                ...
                self.transcription_status.config(
                    text=f"Text saved to {Path(filename).name}"
                )
        except Exception as e:
    ...
    def browse_file(self):
        ...
        filetypes = [
            (
                "All Supported",
                "*.mp3;*.mp4;*.wav;*.flac;*.m4a;*.ogg;*.wma;*.aac;*.avi;*.mkv;*.mov;*.wmv",
            ),
        ...
        filename = filedialog.askopenfilename(
            title="Select Audio/Video File", filetypes=filetypes
        )
    ...
    def update_file_info(self, filename):
        ...
        try:
            ...
            self.file_info_label.config(
                text=f"Error reading file: {e}", foreground="#e74c3c"
            )
...
large: Best accuracy, slowest (~1550 MB)
   ...
   • Quality: Maximum accuracy"""
            ...
            try:
                ...
                success = self.env_manager.setup_environment(
                    progress_callback=lambda msg: self.root.after(
                        0, lambda: self.progress_label.config(text=msg)
                    )
                )
                ...
                if success:
                    ...
                    messagebox.showerror(
                        "Error", "Environment setup failed. Check console for details."
                    )
    ...
    def start_transcription(self):
        ...
        def transcription_worker():
            try:
                ...
                self.root.after(
                    0, lambda: self.start_button.config(text="Transcribing...")
                )
                self.root.after(
                    0,
                    lambda: self.transcription_status.config(
                ...
                if self.user_stopped:
                    self.root.after(
                        0, lambda: self.status_var.set("Transcription stopped by user")
                    )
                    self.root.after(
                        0,
                        lambda: self.transcription_status.config(
                    ...
                    # Show results dialog
                    files_created = "\n".join(
                        [f"• {Path(f).name}" for f in result["files"]]
                    )
                    ...
                    self.root.after(
                        0, lambda: messagebox.showinfo("Success", result_msg)
                    )
                else:
                    error_msg = result.get("error", "Unknown error")
                    self.root.after(
                        0, lambda: self.status_var.set("Transcription failed")
                    )
                    self.root.after(
                        0,
                        lambda: self.transcription_status.config(
                ...
                self.root.after(
                    0, lambda: self.transcription_status.config(text="Error occurred")
                )
                self.root.after(
                    0,
                    lambda: messagebox.showerror(
    ...
    def show_completion_message(self, result):
        ...
        completion_msg += f"{'='*50}\n"
        completion_msg += (
            f"⏱️ Processing time: {result.get('transcription_time', 0):.1f} seconds\n"
        )
    ...
    def pause_transcription(self):
        ...
        if self.is_paused:
            ...
            self.transcription_status.config(
                text="🎙️ Live Transcription Active (Resumed)"
            )
    ...
    def on_closing(self):
        ...
        if self.is_processing:
            if messagebox.askokcancel(
                "Quit", "Transcription in progress. Stop and quit?"
            ):
                self.user_stopped = True
                self.cleanup_processes()
                self.stop_timer()
